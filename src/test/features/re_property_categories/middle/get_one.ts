...
console.log("\n- re_property_middle_categories.getOne");

export const test_success = async (connection: IConnection) => {
  const query = typia.random<IREPropertyCategory.IMiddle.ISearch>();
  const super_categories = await re_property_middle_categories.getList(
    connection,
    query
  );
  const received = await ArrayUtil.asyncMap(super_categories.data)((category) =>
    re_property_middle_categories.getOne(connection, category.id)
  );
...
export const test_not_found = internal.test_error((connection: IConnection) =>
  re_property_middle_categories.getOne(connection, randomUUID())
