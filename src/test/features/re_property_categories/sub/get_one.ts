...
console.log("\n- re_property_sub_categories.getOne");

export const test_success = async (connection: IConnection) => {
  const query = typia.random<IREPropertyCategory.ISub.ISearch>();
  const super_categories = await re_property_sub_categories.getList(
    connection,
    query
  );
  const received = await ArrayUtil.asyncMap(super_categories.data)((category) =>
    re_property_sub_categories.getOne(connection, category.id)
  );
