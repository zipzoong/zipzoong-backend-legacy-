...
console.log("\n- re_property_categories.super.getOne");
...
export const test_success = async (connection: IConnection) => {
  ...
  const received = await ArrayUtil.asyncMap(super_categories.map(pick("id")))(
    (category_id) =>
      re_property_categories.super.getOne(connection, category_id)
  );
...
export const test_not_found = internal.test_error((connection: IConnection) =>
  re_property_categories.super.getOne(connection, randomUUID())
